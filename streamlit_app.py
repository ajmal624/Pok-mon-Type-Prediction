...
# -----------------------------
# Page Config
st.set_page_config(page_title="Pokémon Legendary Predictor", layout="wide")
st.title("Pokémon Legendary Predictor 🐉")
st.write("Predict if a Pokémon is Legendary based on its stats and see its sprite!")
...
with col2:
    st.subheader("Pokémon Legendary Prediction")
    if st.button("Predict Legendary Status"):
        try:
            ...
            if pred_class == 1:
                st.success(f"🏆 {Name} is LEGENDARY! (Probability: {pred_prob:.2f})")
            else:
                st.info(f"{Name} is NOT Legendary. (Probability: {pred_prob:.2f})")
